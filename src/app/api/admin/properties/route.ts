...
const propertySchema = z.object({
  ...
  basePrice: z.preprocess((val) => Number(val), z.number().positive("Preço base deve ser maior que 0")),
  condominiumFee: z.preprocess((val) => Number(val), z.number().min(0).optional().default(0)),
  iptuTax: z.preprocess((val) => Number(val), z.number().min(0).optional().default(0)),
  areaUseful: z.preprocess((val) => Number(val), z.number().positive("Área útil deve ser maior que 0")),
  ...
  suites: z.preprocess((val) => Number(val), z.number().min(0).optional().default(0)),
  parkingSpots: z.preprocess((val) => Number(val), z.number().min(0, "Vagas não pode ser negativo").optional().default(0)),
  isCoveredParking: z.boolean().optional().default(false),
  ...
  state: z.string().min(2, "Estado inválido").optional().default("TO"),
  ...
  try {
    ...
    const property = await prisma.property.create({
      data: {
        ...
        condominiumFee: parsedData.condominiumFee,
        iptuTax: parsedData.iptuTax,
        areaUseful: parsedData.areaUseful,
        areaTotal: parsedData.areaTotal,
        bedrooms: parsedData.bedrooms,
        bathrooms: parsedData.bathrooms,
        suites: parsedData.suites,
        parkingSpots: parsedData.parkingSpots,
        isCoveredParking: parsedData.isCoveredParking,
        ...
        try {
          const activeRadars = await prisma.savedSearch.findMany({
            where: {
              AND: [
                ...
                { OR: [{ maxPrice: { gte: property.basePrice } }, { maxPrice: null }] },
          ...
          if (activeRadars.length > 0 && process.env.VAPID_PRIVATE_KEY) {
            const payload = JSON.stringify({
              ...
              body: `Match perfeito! Novo imóvel em ${property.city} por R$ ${property.basePrice.toLocaleString('pt-BR')} acabou de ser publicado.`,
